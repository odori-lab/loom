...
test.describe('Dashboard - LoomsTab List View & PreviewModal', () => {
  ...
  test('should render looms as list items (not grid)', async ({ page }) => {
    ...
    const loomRows = page.locator('#looms-container [class*="rounded-xl"][class*="cursor-pointer"]')
    const count = await loomRows.count()
    console.log(`Found ${count} loom rows`)
    expect(count).toBeGreaterThanOrEqual(2)

    await page.screenshot({ path: 'test-results/dashboard-01-list-view.png', fullPage: true })
  })

  test('should expand chapters when clicking a loom', async ({ page }) => {
    // Click first loom row
    const firstRow = page.locator('#looms-container [class*="rounded-xl"][class*="cursor-pointer"]').first()
    ...
    await page.screenshot({ path: 'test-results/dashboard-02-chapters-expanded.png', fullPage: true })
  })

  test('should collapse chapters when clicking same loom again', async ({ page }) => {
    // Click to expand
    const firstRow = page.locator('#looms-container [class*="rounded-xl"][class*="cursor-pointer"]').first()
    await firstRow.click()
    await page.waitForTimeout(300)
    ...
    await page.screenshot({ path: 'test-results/dashboard-03-chapters-collapsed.png', fullPage: true })
  })

  test('should show "No chapters available" for loom without book_structure', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/dashboard-04-no-chapters.png', fullPage: true })
  })
  ...
  test('should show selected state on loom row', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/dashboard-05-selected-state.png', fullPage: true })
  })

  test('LoomPreviewPanel should center content vertically', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/dashboard-06-preview-panel.png', fullPage: true })
  })
  ...
  test('PreviewModal should not be full viewport', async ({ page }) => {
    ...
    if (modalVisible) {
      ...
      const innerModal = page.locator('.rounded-2xl.overflow-hidden.shadow-2xl')
      ...
      await page.screenshot({ path: 'test-results/dashboard-07-preview-modal.png', fullPage: true })
  ...
  test('should handle search functionality', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/dashboard-08-search.png', fullPage: true })
