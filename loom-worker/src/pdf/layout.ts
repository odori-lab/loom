...
const ESSAY_MIN_CONTINUATION_HEIGHT = ESSAY_POST_HEADER_HEIGHT + ESSAY_LINE_HEIGHT * 2
...
function estimateEssayTextHeight(content: string): number {
  ...
  let totalLines = 0
  for (const line of lines) {
    const renderedLines = Math.max(Math.ceil(line.length / ESSAY_CHARS_PER_LINE), 1)
    totalLines += renderedLines
  }
...
function splitEssayPostAcrossPages(post: ThreadsPost, currentPageUsed: number): PostChunk[] {
  const chunks: PostChunk[] = []
  const content = post.content
  ...
  while (contentIndex < content.length) {
    ...
    const imageHeight = isFirst && post.imageUrls.length > 0 ? IMAGE_HEIGHT : 0
    const availableForText = availableHeight - headerHeight - imageHeight - ESSAY_POST_MARGIN

    const linesAvailable = Math.max(Math.floor(availableForText / ESSAY_LINE_HEIGHT), 1)
    const endIndex = findEssayEndIndexForLines(content, contentIndex, linesAvailable)
    const isLast = endIndex >= content.length
...
function findEssayEndIndexForLines(content: string, startIndex: number, maxLines: number): number {
  const remaining = content.slice(startIndex)
  const lines = remaining.split('\n')
