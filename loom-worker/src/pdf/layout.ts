...
// Essay mode height estimates
const ESSAY_POST_HEADER_HEIGHT = 24; // Just date + likes, no avatar
const ESSAY_POST_MARGIN = 40; // More generous spacing between posts
const ESSAY_LINE_HEIGHT = 24; // Larger line spacing (1.7 line-height)
...
function buildThreadInfo(
  posts: ThreadsPost[],
): Map<string, { posts: ThreadsPost[]; positions: Map<string, number> }> {
  const threadInfo = new Map<
    string,
    { posts: ThreadsPost[]; positions: Map<string, number> }
  >();
...
export function calculateLayout(posts: ThreadsPost[]): PageLayout[] {
  ...
  for (let i = 0; i < posts.length; i++) {
    ...
    const threadPosition = threadData
      ? threadData.positions.get(post.id)
      : undefined;
    ...
    if (currentHeight + postHeight <= MAX_PAGE_HEIGHT) {
      ...
      currentChunks.push(
        createFullPostChunk(post, threadPosition, threadTotal),
      );
      ...
      const chunks = splitPostAcrossPages(
        post,
        currentHeight,
        threadPosition,
        threadTotal,
...
function splitPostAcrossPages(
  ...
  while (contentIndex < content.length) {
    ...
    const availableForText =
      availableHeight - headerHeight - imageHeight - POST_MARGIN - statsHeight;

    // Calculate how many lines fit
    const linesAvailable = Math.max(
      Math.floor(availableForText / LINE_HEIGHT),
      1,
    );

    // Find end index based on available lines
    const endIndex = findEndIndexForLines(
      content,
      contentIndex,
      linesAvailable,
    );
...
function estimateChunkHeight(chunk: PostChunk): number {
  ...
  const chunkContent = chunk.post.content.slice(
    chunk.contentStart,
    chunk.contentEnd,
  );
...
// Essay mode layout calculation
export function calculateEssayLayout(posts: ThreadsPost[]): PageLayout[] {
  const pages: PageLayout[] = [];
  let currentChunks: PostChunk[] = [];
  let currentHeight = 0;
  ...
  for (let i = 0; i < posts.length; i++) {
    const post = posts[i]!;
    const postHeight = estimateEssayPostHeight(post);

    if (currentHeight + postHeight <= MAX_PAGE_HEIGHT) {
      currentChunks.push(createEssayFullPostChunk(post));
      currentHeight += postHeight;
    } else if (postHeight <= MAX_PAGE_HEIGHT) {
      if (currentChunks.length > 0) {
        pages.push({ posts: [], chunks: currentChunks });
      }
      currentChunks = [createEssayFullPostChunk(post)];
      currentHeight = postHeight;
    } else {
      // Post is too tall - split across pages
      const chunks = splitEssayPostAcrossPages(post, currentHeight);
      for (let j = 0; j < chunks.length; j++) {
        const chunk = chunks[j]!;
        const chunkHeight = estimateEssayChunkHeight(chunk);
...
function createEssayFullPostChunk(post: ThreadsPost): PostChunk {
  return {
    post,
    contentStart: 0,
    contentEnd: post.content.length,
...
function estimateEssayPostHeight(post: ThreadsPost): number {
  let height = ESSAY_POST_HEADER_HEIGHT + ESSAY_POST_MARGIN;
  height += estimateEssayTextHeight(post.content);
  if (post.imageUrls.length > 0) {
    height += IMAGE_HEIGHT;
  }
  return height;
...
function estimateEssayTextHeight(content: string): number {
  const lines = content.split("\n");
  let totalLines = 0;
  for (const line of lines) {
    const renderedLines = Math.max(
      Math.ceil(line.length / ESSAY_CHARS_PER_LINE),
      1,
    );
    totalLines += renderedLines;
  }
  return totalLines * ESSAY_LINE_HEIGHT;
}

function estimateEssayChunkHeight(chunk: PostChunk): number {
  let height = ESSAY_POST_MARGIN;
  if (chunk.showHeader) {
    height += ESSAY_POST_HEADER_HEIGHT;
  }
  if (chunk.showImages && chunk.post.imageUrls.length > 0) {
    height += IMAGE_HEIGHT;
  }
  ...
  height += estimateEssayTextHeight(chunkContent);
  return height;
}

function splitEssayPostAcrossPages(
  post: ThreadsPost,
  currentPageUsed: number,
): PostChunk[] {
  const chunks: PostChunk[] = [];
  ...
  let availableHeight = MAX_PAGE_HEIGHT - currentPageUsed;
  if (availableHeight < ESSAY_MIN_CONTINUATION_HEIGHT) {
    availableHeight = MAX_PAGE_HEIGHT;
  }
  ...
  while (contentIndex < content.length) {
    const headerHeight = isFirst ? ESSAY_POST_HEADER_HEIGHT : 0;
    const imageHeight = isFirst && post.imageUrls.length > 0 ? IMAGE_HEIGHT : 0;
    const availableForText =
      availableHeight - headerHeight - imageHeight - ESSAY_POST_MARGIN;

    const linesAvailable = Math.max(
      Math.floor(availableForText / ESSAY_LINE_HEIGHT),
      1,
    );
    const endIndex = findEssayEndIndexForLines(
      content,
      contentIndex,
      linesAvailable,
    );
...
function findEssayEndIndexForLines(
  content: string,
  startIndex: number,
  maxLines: number,
): number {
  ...
  for (const line of lines) {
    ...
    if (lineCount + renderedLines > maxLines) {
      if (lineCount === 0) {
        const charsForRemainingLines = maxLines * ESSAY_CHARS_PER_LINE;
        let breakPoint = Math.min(charsForRemainingLines, line.length);
        const lastSpace = line.lastIndexOf(" ", breakPoint);
        if (lastSpace > 0) {
          breakPoint = lastSpace + 1;
