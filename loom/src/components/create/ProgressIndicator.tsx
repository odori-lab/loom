...
function getStepStyle(stepIndex: number, currentStepIndex: number, isCurrentStep: boolean): string {
...
export function ProgressIndicator({ compact = false }: { compact?: boolean }) {
  ...
  const sizeClass = compact ? 'w-6 h-6 text-xs rounded-lg' : 'w-8 h-8 text-sm rounded-xl'
    ...
    <div className={`flex items-center ${compact ? 'gap-1.5' : 'gap-2'}`}>
      {steps.map((s, i) => (
        <div key={s} className="flex items-center">
          ...
          {i < 2 && (
            <div className={`mx-1 ${connectorClass} transition-colors duration-300 ${
