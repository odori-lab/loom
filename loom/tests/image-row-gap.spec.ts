...
async function measureImageGaps(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    const allImages = Array.from(
      document.querySelectorAll(".essay-inline-image"),
    );
...
test.describe("Image row gap - 6px target", () => {
  ...
  test("single figure with 6 images: gap should be 6px", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/gap-single-figure.png",
      fullPage: true,
    });
  ...
  test("3 separate figures, no headers: gap should be 6px", async ({
    page,
  }) => {
    const pairs = [
      ["#e74c3c", "#3498db"],
    ...
    await page.screenshot({
      path: "test-results/gap-adjacent-figures.png",
      fullPage: true,
    });
    expect(gaps.length).toBe(2);
    for (const g of gaps) expect(g).toBeCloseTo(6, 0);
  ...
  test("figures with post-headers between: gap should be 6px (headers hidden)", async ({
    page,
  }) => {
    const pairs = [
      ["#e74c3c", "#3498db"],
    ...
    const hiddenHeaders = await page.evaluate(() => {
      const headers = Array.from(
        document.querySelectorAll(".essay-post-header"),
      );
    ...
    await page.screenshot({
      path: "test-results/gap-figures-with-headers.png",
      fullPage: true,
    });
    expect(gaps.length).toBe(2);
    for (const g of gaps) expect(g).toBeCloseTo(6, 0);
  ...
  test("posts with text should keep normal spacing", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/gap-text-and-images.png",
      fullPage: true,
    });
  });
});
