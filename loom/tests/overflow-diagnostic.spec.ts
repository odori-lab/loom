...
test.describe('Overflow Diagnostic - Detailed Analysis', () => {
  ...
  test('diagnose overflow root causes across all spreads', async ({ page }) => {
    ...
    for (let i = 0; i < spreadCount; i++) {
      ...
      for (let j = 0; j < iframeCount; j++) {
        const result = await page.evaluate(async (idx) => {
          ...
          const childHeights = children.map(c => Math.ceil(c.getBoundingClientRect().height))
          ...
          const hasSubChapter = pageDiv.querySelectorAll('.essay-sub-chapter-title, [class*="sub-chapter"]').length
        ...
        if (result) {
          ...
          console.log(`\n--- Spread ${i}, Page ${j} ---`)
          console.log(`  Overflow: ${result.overflow}px (${result.scrollHeight} > ${result.clientHeight})`)
          console.log(`  Children: ${result.childCount}, Heights: [${result.childHeights.join(', ')}]`)
          console.log(`  Images: ${result.hasImages}, Essay posts: ${result.hasEssayPost}, Sub-chapters: ${result.hasSubChapter}`)
          console.log(`  CSS: height=${result.pageHeight}, overflow=${result.pageOverflow}, minHeight=${result.pageMinHeight}`)
    ...
    const moderate = overflowDetails.filter(d => d.overflow > 30 && d.overflow <= 200)
    ...
    if (severe.length > 0) {
      console.log(`\n  Severe overflows:`)
      severe.forEach(d => {
        console.log(`    Spread ${d.spread}, Page ${d.page}: +${d.overflow}px, ${d.childCount} children`)
      })
    }
  })
})
